...
public class ZombieCrushSagaZombie extends Sprite {
    ...
    // IF IT IS SPECIAL, WILL BE DEFINED AS SPECIAL_WRAPPED OR SPECIAL_STRIPED 
    // STATE.  OTHERWISE WILL RETURN WITH "BASIC".
     ...
     * otherwise.
     */
    public boolean match(ZombieCrushSagaZombie testTile) {
        return true;
    }
    ...
    public void update(MiniGame game) {
        ...
        if (((ZombieCrushSagaMiniGame) game).inProgress) {
            vel = MAX_IN_GAME_VELOCITY;
        } else {
            vel = MAX_TILE_VELOCITY;
        }
        // IF WE ARE IN A POST-WIN STATE WE ARE PLAYING THE WIN
        // ANIMATION, SO MAKE SURE THIS TILE FOLLOWS THE PATH
        /**
         * if (game.getDataModel().won()) { updateWinPath(game); }
         *
         * // IF NOT, IF THIS TILE IS ALMOST AT ITS TARGET DESTINATION, // JUST
         * GO TO THE TARGET AND THEN STOP MOVING
         *
         * else
         *
         */
        if (calculateDistanceToTarget() < vel) {
            vX = 0;
            vY = 0;
            x = targetX;
            y = targetY;
